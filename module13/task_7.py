...
precision = float(input('Введите максимально допустимый уровень опасности: '))
...
while True:
  diff = (top + down) / 2
  if abs(danger(diff)) < precision:
    print('Приблизительная глубина безопасной кладки: ',diff)
    break
  elif danger(diff) < precision:
    top = diff  - 1
  elif danger(diff) > precision:
    down = diff + 1