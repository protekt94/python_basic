...
def calculation(cred,per,yrs,flag):
  for i in range(1,yrs+1):
    print('Период {}'.format(i))
    print()
    print('Остаток долга на начало периода:',cred)
    k = (per * (1 + per) ** yrs) / ((1 + per) ** yrs - 1) 
    paid_per = cred * per
    a = cred * k - paid_per
    cred -= a
    yrs -= 1
    print('Выплачено процентов:',paid_per)
    print('Выплачено тела кредита:',a)
    print()
    if i == 3 and flag:
      print('Остаток долга',cred)
      print('='* 20)
      years_2 = int(input('На сколько лет продлить договор? '))
      percent_2 = float(input('Увеление ставки до: '))
      calculation(cred, percent_2, years_2+yrs, False)
      break
    print('Остаток долга',cred)
      
calculation(credit, percent, years,True)
