...
time = 0
summ = 0
for num in range(math.ceil(file/speed)):
  time += 1
  summ += speed
  if summ > file:
    summ = file
  percent = math.ceil(summ // (file/100))
  print('Прошло {} сек. Скачано {} из {} Мб ({}%)'\
        .format(time,summ,file,percent))
print('Всего скачивание обновления заняло {} сек.'.format(time))
